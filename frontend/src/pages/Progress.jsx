...
import { Loader2, FileText, BarChart2, Map, PenTool, BookOpen, Headset, Edit3, Mic, TrendingUp, ArrowRight } from 'lucide-react'
    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          disabled={exporting}
          className="bg-primary-600 hover:bg-primary-500 text-white px-5 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 shadow-lg"
        >
          {exporting ? <><Loader2 className="w-4 h-4 animate-spin" /> Exporting...</> : <><FileText className="w-4 h-4" /> Export Report</>}
        </button>
      </div>

      <div ref={reportRef} className="space-y-6 p-4 rounded-2xl bg-slate-900/50">

      {/* Tabs */}
      <div className="flex gap-2">
        <button onClick={() => setTab('stats')} className={`px-5 py-3 rounded-xl font-semibold flex items-center gap-2 transition-all ${tab === 'stats' ? 'bg-primary-600 text-white shadow-lg' : 'bg-white/5 text-primary-300 hover:bg-white/10'}`}>
          <BarChart2 className="w-5 h-5" /> Statistics
        </button>
        <Link to="/roadmap" className="px-5 py-3 rounded-xl font-semibold bg-white/5 text-primary-300 hover:bg-white/10 flex items-center gap-2 transition-all hover:text-white">
      ...
      {tab === 'stats' && (
        <>
          ...
          {progress?.overall && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="glass-card p-5 text-center">
                <p className="text-3xl font-bold text-primary-400">{progress.overall.total_questions || 0}</p>
                <p className="text-sm text-primary-300 mt-1">Total Questions</p>
              </div>
              <div className="glass-card p-5 text-center">
                <p className="text-3xl font-bold text-success-400">{progress.overall.correct_answers || 0}</p>
                <p className="text-sm text-primary-300 mt-1">Correct</p>
              </div>
              <div className="glass-card p-5 text-center col-span-2 sm:col-span-1">
                <p className="text-3xl font-bold text-accent-400">{progress.overall.accuracy || 0}%</p>
          ...
          <div className="glass-card p-6">
            <h3 className="text-lg font-bold text-white mb-6">Skill Weakness Heatmap</h3>
            <div className="overflow-x-auto">
              <div className="min-w-[500px]">
                <div className="grid grid-cols-6 gap-2 mb-2">
                  <div className="text-xs font-bold text-primary-400 uppercase tracking-tighter">Skill / Part</div>
                  {[1, 2, 3, 4, 5].map(p => (
                    <div key={p} className="text-center text-xs font-bold text-primary-400 uppercase">Part {p}</div>
                  ))}
                </div>
                {['grammar', 'reading', 'listening', 'writing', 'speaking'].map(skill => (
                  <div key={skill} className="grid grid-cols-6 gap-2 mb-2 items-center">
                    <div className="text-sm font-medium text-white capitalize">{skill}</div>
                    {[1, 2, 3, 4, 5].map(part => {
          ...
          {progress?.skills?.length > 0 && (
            <div className="grid gap-3">
              {progress.skills.map(s => (
                <div key={s.skill} className="glass-card p-4 flex items-center gap-4 hover:bg-white/5 transition-colors">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      <span className="font-semibold text-white capitalize">{s.skill}</span>
                      <span className="text-primary-300 text-sm">{s.correct_answers}/{s.total_questions} correct</span>
                    </div>
                    <div className="w-full bg-white/10 rounded-full h-2">
                      <div className={`h-2 rounded-full skill-${s.skill}`} style={{ width: `${s.accuracy}%` }} />
                    </div>
                  </div>
                  <span className="text-xl font-bold text-white">{s.accuracy}%</span>
                </div>
              ))}
            </div>
          )}
          ...
          {(!progress?.skills || progress.skills.length === 0) && (
            <div className="glass-card p-8 text-center flex flex-col items-center justify-center">
              <TrendingUp className="w-16 h-16 text-primary-500/50 mb-4" />
              <p className="text-primary-300 text-lg">No progress data yet. Start practising to see your stats!</p>
            </div>
          )}
        </>
      )}
      ...
      {tab === 'roadmap' && (
        <div className="glass-card p-8 text-center flex flex-col items-center justify-center">
          <Map className="w-16 h-16 text-primary-500/50 mb-6" />
          <h2 className="text-2xl font-bold text-white mb-2">Go to your Roadmap</h2>
          <p className="text-primary-300 mb-8 max-w-md mx-auto text-lg">Your full A1-A2 → B1-B2 learning path with session unlocking is on the dedicated Roadmap page.</p>
          <Link to="/roadmap" className="btn-glow bg-gradient-to-r from-emerald-600 to-primary-600 text-white px-8 py-3.5 rounded-xl font-bold text-lg flex items-center justify-center gap-2 shadow-lg hover:from-emerald-500 hover:to-primary-500 transition-all">
