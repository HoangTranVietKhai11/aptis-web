...
const insertQuery = db.prepare('INSERT INTO vocabulary (word, definition, example_sentence, part_of_speech, level, theme) VALUES (?, ?, ?, ?, ?, ?)');
...
db.transaction(() => {
  ...
  for (const line of lines) {
    ...
    if (word && theme) {
      insertQuery.run(word, def, ex, pos, level, theme);
      cnt++;
    }
  }
  console.log('Added ' + cnt + ' vocab words!');
})();
db.close();
